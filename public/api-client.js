 ...
 */
(function () {
  // ✅ FIXED: Points to the actual backend URL (not localhost!)
  const API_BASE = 
    window.KLOYYA_API_BASE ||  // Allow override via window variable
    (typeof window !== 'undefined' && window.location.hostname === 'localhost' 
      ? "http://localhost:3001"  // Local dev backend
      : "https://kloyya-ia-vert.vercel.app");  // Production backend

  // Log for debugging
  console.log("[KloyyaAPI] Initialized with API_BASE:", API_BASE);
  ...
  async function request(path, options = {}) {
    ...
    console.log(`[KloyyaAPI] ${options.method || 'GET'} ${fullUrl}`);
    
    const res = await fetch(fullUrl, { ...options, headers });
    if (!res.ok) {
      const body = await res.text();
      console.error(`[KloyyaAPI] Error ${res.status}:`, body);
  ...
  console.log("[KloyyaAPI] Ready! Call window.KloyyaAPI.* from your frontend.");
})();
